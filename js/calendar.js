...
(function(window) {
    ...
    window.CalendarManager = {
        ...
        changeCategory: async function(dateStr, slotId, newCategory) {
            await window.updateMenuForDate(dateStr, slotId, newCategory, null);
            this.render();
        },

        selectRecipe: async function(dateStr, slotId, recipeId) {
            const menu = window.getMenuForDate(dateStr);
            const currentCategory = menu[slotId]?.category || this.getDefaultCategory(parseInt(slotId.replace('slot', '')));
            await window.updateMenuForDate(dateStr, slotId, currentCategory, recipeId || null);
    ...
    window.goToToday = function() { window.CalendarManager.goToToday(); };
