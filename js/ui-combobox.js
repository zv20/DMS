 ...
 *
 * 2. window.initComboboxOnElement({ inputEl, dropdownEl, getItems, onSelect, placeholder })
...
(function(window) {
    ...
    function attachCombobox(inp, dropdown, getItems, onSelect, placeholder) {
        ...
        function selectItem(item) {
            onSelect(item);
            inp.value = item.label;   // show selected name in the field
        ...
        inp.addEventListener('keydown', e => {
            ...
            if (e.key === 'ArrowDown')  { e.preventDefault(); setActive(Math.min(activeIndex + 1, lis.length - 1)); }
    ...
    window.initCombobox = function({ inputId, dropdownId, getItems, onSelect, placeholder }) {
        ...
        attachCombobox(document.getElementById(inputId), dropdown, getItems, onSelect, placeholder);
    };

    // --- Entry point 2: Element-based (dynamic calendar slots) ---
    window.initComboboxOnElement = function({ inputEl, dropdownEl, getItems, onSelect, placeholder, selectedId, selectedLabel }) {
        ...
        attachCombobox(inputEl, dropdownEl, getItems, onSelect, placeholder);
    };
    ...
    (function injectStyles() {
        ...
        style.textContent = `
            ...
            /* Calendar slot dropdowns — contained inside .meal-slot */
